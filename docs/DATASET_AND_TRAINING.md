...
| **WLASL** | Optional small supplement from the public WLASL index. Used only for direct MP4 URLs, primarily to backfill signs missing from ASL Citizen such as `five`. YouTube entries are skipped by default. | Fetched on demand by [`ml/scripts/fetch_wlasl.py`](../ml/scripts/fetch_wlasl.py) when `include_wlasl=true`; never committed. |
| **ASLLVD** | Optional targeted supplement from Boston University's ASLLVD citation-form annotations. Used for exact-gloss clips such as `five`; scene videos are downloaded and trimmed locally from annotated frame ranges. Use only for research/education and do not redistribute the raw clips. | Fetched on demand by [`ml/scripts/fetch_asllvd.py`](../ml/scripts/fetch_asllvd.py) when `include_asllvd=true`; never committed. |
| **PopSign ASL** | Optional learner-style supplement from Georgia Tech's PopSign ASL dataset, CC BY 4.0. The fetcher streams only requested per-sign tar files and stops after a clip cap; use it for targeted signs PopSign contains, such as `hello`, `goodbye`/`bye`, `please`, `thank_you`/`thankyou`, `yes`, `no`, `where`, `who`, `sleep`, and `water`. | Fetched on demand by [`ml/scripts/fetch_popsign.py`](../ml/scripts/fetch_popsign.py); never committed. |
