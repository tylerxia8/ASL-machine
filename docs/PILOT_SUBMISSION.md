...
Current model: `wave1-semlex-aslcitizen-wlasl-v27-v23recipe`, bundled in `apps/web/public/models/` and published as a GitHub Release. It is trained from scratch on Sem-Lex plus targeted ASL Citizen/WLASL supplements and reaches 81.84% signer-disjoint clip-level test accuracy across the 25-sign Wave 1 roster. Remaining weak spots are `five`, `four`, and `who`; see the validation and capture-plan docs for details.
...
| 2 | Trained sign recognition model | Published as a GitHub Release: https://github.com/tylerxia8/ASL-machine/releases/tag/wave1-semlex-aslcitizen-wlasl-v27-v23recipe. The same `model.onnx`, `labels.json`, `model_meta.json`, and `recognition_calibration.json` are bundled in `apps/web/public/models/`. |
