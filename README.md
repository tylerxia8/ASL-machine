...
## Wave 1 workflow (recommended)

1. **Shakedown first** — record 5 clips/sign for 5 signs and run the full pipeline. See [docs/WAVE1_SHAKEDOWN.md](docs/WAVE1_SHAKEDOWN.md). Catches pipeline bugs in 20 minutes instead of after a 4-hour recording session.
2. **Collect** — http://localhost:5173/capture (25 trained signs, target 30 clips/sign × 2 signers). Move `.webm` downloads to `ml/data/incoming/` (or run `scripts\import-from-downloads.ps1`).
3. **Retrain** — `.\scripts\continue-wave1.ps1` from repo root.
4. **Practice** — Lobby → **Start Wave 1 session**.
5. **Dry run** — walk [docs/WAVE1_DRY_RUN.md](docs/WAVE1_DRY_RUN.md) with 2–3 testers.
